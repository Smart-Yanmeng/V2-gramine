...
class NetworkClient (Process):
    ...
    def _connect_and_send_forever(self):
        ...
        while True:
            try:
                if not self.is_out_sock_connected[0]:
                    self.is_out_sock_connected[0] = self._connect(0)
                if not self.is_out_sock_connected[self.id - 1]:
                    self.is_out_sock_connected[self.id - 1] = self._connect(self.id - 1)
                if self.is_out_sock_connected[0] and self.is_out_sock_connected[self.id - 1]:
                    break
